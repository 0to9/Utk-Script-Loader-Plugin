...
import io.github.utk003.spigot_scripting.util.exception.CompilingException;
...
public class ScriptLinker {
    ...
    public static void linkScripts() {
        try {
            ...
            for (int i = 0; i < 10; i++) {
                lambdaClassSet.add(classPool.get("io.github.utk003.util.function.void_lambda.VoidLambda" + i));
                lambdaClassSet.add(classPool.get("io.github.utk003.util.function.lambda.Lambda" + i));
            }
            ...
            Logger.logError("Encountered an error while linking scripts", e);
        }
    }
    ...
    private static CtMethod getCommandMethod(Command command, Script script,
                                             Map<DataHolder, Pair<String, CtClass>> classMap) throws Exception {
        StringBuilder jointId = new StringBuilder();
        for (String id : command.COMMAND_IDS)
            jointId.append("_").append(id);

        String commandMethodName = "subCommandHandler__" + command.TYPE + "__" + jointId.substring(1);
        String returnType = command.TYPE.toMethodReturnType();
        ...
        Pair<String, CtClass> holder = classMap.get(script);
        holder.second.addMethod(method);
        CommandUtil.addSubCommandHandler(command.COMMAND_IDS, holder.second, commandMethodName, command.TYPE);
        return method;
    }
            ...
            switch (token) {
                case "$":
                    ...
                    switch (tok) {
                        ...
                        case "log":
                        case "logError":
                            builder.append(Logger.PATH).append(".").append(tok);
                            break;

                        case "PLUGIN":
                        case "JAVA_PLUGIN":
                            builder.append(PluginMain.PATH).append(".PLUGIN");
                            break;
